\section{波动光学通论 (General Wave Optics)}
...
\begin{enumerate}
    ...
    等相面方程：$\vec{k} \cdot \vec{r} = \text{const}$（平面）。
...
\mydef{远场条件 (Far-field Condition)}
通常指观察距离 $z$ 远大于波源或孔径尺寸 $D$ 的平方与波长的比值的情况。
\begin{itemize}
    \item \textbf{判据}：$z \gg \frac{D^2}{\lambda}$（或定义菲涅耳数 $N_F = \frac{D^2}{\lambda z} \ll 1$）。
...
\mydef{小角度干涉条件 (Small-angle Interference Condition)}
当两束相干光的传播方向之间的夹角 $\alpha$ 很小时（$\alpha \ll 1$）：
\begin{itemize}
    \item \textbf{矢量近似}：严格的干涉项含 $\vec{E}_1 \cdot \vec{E}_2 = E_1 E_2 \cos \alpha$。当 $\alpha \to 0$ 时，$\cos \alpha \to 1$。
    \item \textbf{结论}：只要干涉角足够小，即使不考虑矢量的偏振方向（假设它们大体同向），\textbf{标量叠加原理}也是高度精确的。
\end{itemize}

\mydef{自然光的分解模型 (Decomposition Model of Natural Light)}
...
\begin{itemize}
    ...
    \item \textbf{干涉表现}：在杨氏双缝或薄膜干涉中，$x$ 分量与自身的副本干涉，$y$ 分量与自身的副本干涉。由于两者不相干，总光强分布是两组干涉条纹强度的简单叠加。
\end{itemize}

\end{spacing}

\section{光的干涉 (Interference)}
...
\mydef{驻波 (Standing Wave)}
两列振幅相同、相向传播的相干波叠加形成驻波。维纳 (Wiener) 实验证明了光波中的感光作用主要是由**电场矢量**引起的（波腹处感光最强）。
\end{spacing}

\subsection{分波前干涉 (Wavefront Splitting)}
物理上将点光源的波前分割为两部分，分别通过不同路径后汇聚。

\subsubsection{杨氏双缝干涉 (Young's Experiment)}
\textbf{装置参数}：缝间距 $d$，屏距 $D$，波长 $\lambda$。
\textbf{光程差} $\Delta \approx d \frac{x}{D}$。
\textbf{光强分布}：
\begin{equation}
    I(x) = 4I_0 \cos^2 \left( \frac{\pi d x}{D \lambda} \right)
\end{equation}
\textbf{特征}：
\begin{itemize}
...
\begin{itemize}
    \item \textbf{菲涅耳双棱镜}：利用折射产生两个虚光源。
    \item \textbf{菲涅耳双面镜}：利用反射产生两个虚光源。
    \item \textbf{洛埃镜 (Lloyd's Mirror)}：利用直射光与反射光干涉。
    \textbf{重要现象}：在镜面边缘接触点处（光程差趋于0），出现**暗条纹**。这直接证明了光在光密介质表面反射时存在**半波损失**（相位突变 $\pi$）。
\end{itemize}

\subsubsection{空间相干性 (Spatial Coherence)}
实际光源具有宽度 $b$。
\begin{itemize}
...
\subsection{分振幅干涉 (Amplitude Splitting)}
...
\begin{itemize}
    \item \textbf{增透膜 (Anti-reflection Coating)}：利用干涉相消。单层膜条件：$n_{膜}h = \lambda/4$, $n_{膜} = \sqrt{n_{基}}$。
    \item \textbf{高反膜 (High-reflection Coating)}：利用干涉相长。多层介质膜堆叠。
\end{itemize}
...
\subsubsection{时间相干性 (Temporal Coherence)}
实际光源非单色（谱宽 $\Delta \nu$ 或 $\Delta \lambda$）。
...
\subsection{多光束干涉 (Multi-beam Interference)}

\subsubsection{法布里-珀罗 (Fabry-Perot) 干涉仪}
由两块平行的高反射率（$R \to 1$）平板组成。
\textbf{原理}：入射光在腔内多次反射，形成振幅递减、相位等差的无穷多束光叠加。

\subsubsection{艾里公式 (Airy Formula)}
透射光强 $I_T$ 随相位差 $\delta = \frac{4\pi n h \cos i}{\lambda}$ 的分布：
\begin{equation}
    I_T = I_{\max} \frac{1}{1 + F \sin^2 (\delta/2)}
\end{equation}
\textbf{精细度系数 (Coefficient of Finesse)}：$F = \frac{4R}{(1-R)^2}$。
\begin{itemize}
...
\begin{itemize}
    \item \textbf{自由光谱范围 (Free Spectral Range, FSR)}：相邻两个干涉级次之间的波长差。$\Delta \lambda_{fsr} \approx \frac{\lambda^2}{2nh}$。
    \item \textbf{分辨本领 (Resolving Power)}：$\mathcal{R} = \frac{\lambda}{\delta \lambda} = k \mathcal{N}$，其中 $\mathcal{N}$ 为有效光束数，正比于精细度 $\mathcal{F} = \frac{\pi\sqrt{F}}{2}$。
\end{itemize}
