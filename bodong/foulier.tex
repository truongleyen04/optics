...
\textbf{物理意义}：逆变换公式表明，任意复杂的波场 $f(x, y)$ 都可以看作是无穷多个不同方向、不同振幅和相位的平面波 $e^{\ii 2\pi (ux + vy)}$ 的线性叠加。$F(u, v)$ 即为这些平面波的复振幅权重（角谱）。
...
\begin{itemize}
    ...
    这构成了线性系统理论的基础：输出 = 输入 * 脉冲响应 $\implies$ 输出频谱 = 输入频谱 $\cdot$ 传递函数。
\end{itemize}
